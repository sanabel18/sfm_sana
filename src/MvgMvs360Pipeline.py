...
Photogrammetry reconstruction with these steps:
    ...
    10. Export to openMVS               openMVG_main_openMVG2openMVS
    11. Densify point-cloud            DensifyPointCloud
    12. Reconstruct the mesh           ReconstructMesh
    13. Refine the mesh                RefineMesh
    14. Texture the mesh               TextureMesh
...
optional arguments:
  ...
  --preset PRESET           steps list preset in
                            SEQUENTIAL = [0, 1, 2, 3, {9}, 10, 11, 12, 13, 14]
                            GLOBAL = [0, 1, 2, 4, {9}, 10, 11, 12, 13, 14]
...
PRESET = {'SEQUENTIAL_360': [0, 1, 2, 3, 9, 10, 11, 12, 13, 14, 17, 18, 19, 20, 21],
          'SEQUENTIAL_FISHEYE': [0, 1, 2, 3, 10, 11, 12, 13, 14, 17, 19, 20, 21],
          'MVS_360': [9, 10, 11, 12, 13, 14, 17, 18, 19, 20, 21],
          'MVS_FISHEYE': [10, 11, 12, 13, 14, 17, 19, 20, 21],
          'GLOBAL': [0, 1, 2, 4, 9, 10, 11, 12, 13, 14],
          'MVG_SEQ': [0, 1, 2, 3, 5, 6, 7],
          'MVG_GLOBAL': [0, 1, 2, 4, 5, 6, 7],
          'MVS_SGM': [15, 16],
...
class StepsStore:
    ...
    def __init__(self):
        self.steps_data = [
            ...
            # -f = 1 (focal in pixels, no default, 1 = for spherical camera)
            # -c = 5 (fisheye, 7 = spherical camera)
            # -g = 1 (default, 0 = different intrinsics for all images)
            ["Intrinsics analysis",          # 0
             os.path.join(OPENMVG_BIN, "openMVG_main_SfMInit_ImageListing"),
             ["-i", "%input_dir%", "-o", "%matches_dir%", "-c", "7", "-f", "1", "-d", "%camera_file_params%"] if PRESET_DEFAULT == 'SEQUENTIAL_360' else
             ["-i", "%input_dir%", "-o", "%matches_dir%", "-c", "5", "-f", "1000", "-g", "1", "-d", "%camera_file_params%"]],

            # -p = HIGH (high preset, default NORMAL)
            # -n = 30 (number of threads)
            ["Compute features",             # 1
             os.path.join(OPENMVG_BIN, "openMVG_main_ComputeFeatures"),
             ["-i", "%matches_dir%/sfm_data.json", "-o", "%matches_dir%", "-m", "AKAZE_FLOAT", "-p", "ULTRA", "-n", "30"]],

            # -g a (essential matrix with an angular parametrization, default -f = fundamental matrix)
            # -v 3 (video matching with previous 3 frames, -1 = consider all frames)
            ["Compute matches",              # 2
             os.path.join(OPENMVG_BIN, "openMVG_main_ComputeMatches"),
             ["-i", "%matches_dir%/sfm_data.json", "-o", "%matches_dir%", "-n", "HNSWL2", "-r", ".8", "-g", "a", "-v", "10"] if PRESET_DEFAULT == 'SEQUENTIAL_360' else
             ["-i", "%matches_dir%/sfm_data.json", "-o", "%matches_dir%", "-n", "HNSWL2", "-r", ".8", "-g", "f", "-v", "60"]],  # 90 for img_VID

            # -c = 5 (fisheye, 7 = spherical camera)
            ["Incremental reconstruction",   # 3
             os.path.join(OPENMVG_BIN, "openMVG_main_IncrementalSfM"),
             ["-i", "%matches_dir%/sfm_data.json", "-m", "%matches_dir%", "-o", "%reconstruction_dir%", "-c", "7"] if PRESET_DEFAULT == 'SEQUENTIAL_360' else
             ["-i", "%matches_dir%/sfm_data.json", "-m", "%matches_dir%", "-o", "%reconstruction_dir%", "-c", "5"]],
            ...
            ["Spherical to Cubic",           # 9
             os.path.join(OPENMVG_BIN, "openMVG_main_openMVGSpherical2Cubic"),
             ["-i", "%reconstruction_dir%/sfm_data.bin", "-o", "%reconstruction_dir%"]],

            ["Export to openMVS",            # 10
             os.path.join(OPENMVG_BIN, "openMVG_main_openMVG2openMVS"),
             ["-i", "%reconstruction_dir%/sfm_data_perspective.bin", "-o", "%mvs_dir%/scene.mvs", "-d", "%mvs_dir%/images"] if PRESET_DEFAULT == 'SEQUENTIAL_360' else
             ["-i", "%reconstruction_dir%/sfm_data.bin", "-o", "%mvs_dir%/scene.mvs", "-d", "%mvs_dir%/images"]],

            # --- MVS ---

            ["Densify point cloud",          # 11
             os.path.join(OPENMVS_BIN, "DensifyPointCloud"),
             ["scene.mvs", "--dense-config-file", "Densify.ini", "--resolution-level", "1", "-w", "%mvs_dir%", "--number-views", "0", "--number-views-fuse", "6", "--process-priority", "0"]],

            ["Reconstruct the mesh",         # 12
            ...
            ["Convert SfM data format",          # 17
             os.path.join(OPENMVG_BIN, "openMVG_main_ConvertSfM_DataFormat"),
             ["binary", "-i", "%reconstruction_dir%/sfm_data.bin", "-o", "%reconstruction_dir%/sfm_data_all.json", "-V", "-I", "-E"]],

            ["Convert SfM data format (perspective)",          # 18
             os.path.join(OPENMVG_BIN, "openMVG_main_ConvertSfM_DataFormat"),
             ["binary", "-i", "%reconstruction_dir%/sfm_data_perspective.bin", "-o", "%reconstruction_dir%/sfm_data_prespective_all.json", "-V", "-I", "-E"]],

            ["Export keypoint svg files",          # 19
             os.path.join(OPENMVG_BIN, "openMVG_main_exportKeypoints"),
             ["-i", "%reconstruction_dir%/sfm_data.bin", "-o", "%keypoints_svg_dir%", "-d", "%matches_dir%"]],

            ["Export feature match svg files",          # 20
             os.path.join(OPENMVG_BIN, "openMVG_main_exportMatches"),
             ["-i", "%reconstruction_dir%/sfm_data.bin", "-o", "%matches_svg_dir%", "-d", "%matches_dir%", "-m", "%matches_dir%/matches.f.bin"]],

            ["Export track files",          # 21
             os.path.join(OPENMVG_BIN, "openMVG_main_exportTracks"),
             ["-i", "%reconstruction_dir%/sfm_data.bin", "-o", "%tracks_svg_dir%", "-d", "%matches_dir%", "-m", "%matches_dir%/matches.f.bin"]]
            ]
    ...
    def apply_conf(self, conf):
        ...
        for s in self.steps_data:
            ...
            for o in s[2]:
                ...
                co = co.replace("%keypoints_svg_dir%", conf.keypoints_svg_dir)
                co = co.replace("%matches_svg_dir%", conf.matches_svg_dir)
                co = co.replace("%tracks_svg_dir%", conf.tracks_svg_dir)
...
CONF.keypoints_svg_dir = os.path.join(CONF.matches_dir, "keypoints_svg")
CONF.matches_svg_dir = os.path.join(CONF.matches_dir, "matches_svg")
CONF.tracks_svg_dir = os.path.join(CONF.matches_dir, "tracks_svg")
...
mkdir_ine(CONF.keypoints_svg_dir)
mkdir_ine(CONF.matches_svg_dir)
mkdir_ine(CONF.tracks_svg_dir)
