...
class StepsStore:
    ...
    def __init__(self):
        self.steps_data = [
            ...
            # -f 是相機 focal length in pixel，影片截圖沒有 EXIF 要手動算，計算方法參考:
            # https://openmvg.readthedocs.io/en/latest/software/SfM/SfMInit_ImageListing/#automatic-pixel-focal-computation-from-jpeg-exif-metadata
            #
            # 鏡頭 focal length:
            #   iPhone 11 Pro：1.54mm (0.5x), 4.25mm (1x), 6mm (2x)
            #
            # Sensor:
            #   iPhone XS: 5.6 x 4.2mm active area
            #   不確定 iPhone 11 sensor size，假設跟 iPhone XS 一樣
            #
            # 影片 1920x1080，算出 focal length in pixel = max(1920, 1080) * 4.25 / 4.2 = 1943
            #
            ["Intrinsics analysis",          # 0
             os.path.join(OPENMVG_BIN, "openMVG_main_SfMInit_ImageListing"),
             ["-i", "%input_dir%", "-o", "%matches_dir%", "-d", "%camera_file_params%", "-f", "1943"]],
